...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
    ...
    @Query("SELECT c FROM Conversation c WHERE (c.user1.id = :userId OR c.user2.id = :userId) " +
            "AND c.lastMessageAt IS NOT NULL ORDER BY c.lastMessageAt DESC")
    ...
    // Búsquedas por fecha
    @Query("SELECT c FROM Conversation c WHERE (c.user1.id = :userId OR c.user2.id = :userId) " +
            "AND c.lastMessageAt > :since ORDER BY c.lastMessageAt DESC")
    ...
    @Query("SELECT c FROM Conversation c WHERE (c.user1.id = :userId OR c.user2.id = :userId) " +
            "AND c.lastMessageAt IS NOT NULL ORDER BY c.lastMessageAt DESC LIMIT :limit")
